...
{
public:
	Wire(const std::string& = "Wire");
	Wire(std::initializer_list<Signal> list, const std::string& nm);


	virtual Signal	get_signal(ckt_time) const;		// Retrieve a signal.
	virtual void	send_signal(Signal);			// Put & propagate sig.
	virtual void	show_signals() const;			// Show wire signals.
	void	display(std::ostream &, int) const;		// Show wire info.
