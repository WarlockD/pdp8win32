 ...
 */
Port::Port(Connector &con, const std::string&nm) :
 ...
 */
void
Port::display(std::ostream &os, int tabs) const
{
	generate_tabs indent(tabs);

	os << indent << "Port's external connector: "
		<< external->get_name() << std::endl;
 ...
 * component.
 */
Input::Input(Component &cmp, Connector &con, const std::string& n) :
	Port(con, n)
{
...
Input::send_signal(Signal)
{
	...
	std::cerr << "Warning: Cannot send a signal via Input port.\n";
}
 ...
 * component.
 */
Output::Output(Component &cmp, Connector &con, const std::string& n) :
	Port(con, n)
{
	// Add Output port to component's linked list of Output ports
	// pointers.  Note that add takes a constant reference
	// parameter.  Therefore, we cannot pass 'this' explicity.
	cmp.add_output_port(*this);
}
